...
Public Class Main
    ...
    Public Path As String = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().GetName().CodeBase().Remove(0, "file:\\\".Length).Replace("/", "\")) + "\motion\"
    ...
    Public OleDbConnection1 As New System.Data.OleDb.OleDbConnection(db)
    Public odbcDatabase As New System.Data.OleDb.OleDbDataAdapter()
    ...
    Public Sub Create(ByVal Parent As SkyCD.Form1) Implements SkyCD.iInterfacePlugIn.Create
        ...
        With Parent
            ...
            .ToolsToolStripMenuItem.DropDownItems.Add("Motion Detector", New System.Drawing.Bitmap(16, 16), New System.EventHandler(AddressOf Me.OnClick)).MergeIndex = 0
    ...
    Public Sub OnClick2(ByVal sender As Object, ByVal e As System.EventArgs)
    ...
    Public Sub OnClick(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            Dim FileName As System.String = Me.Path & "motion.exe"
            If ((New System.IO.FileInfo(FileName)).Exists) Then
                ChDir(Me.Path)
                Dim pr As System.Diagnostics.Process = System.Diagnostics.Process.Start(FileName)                
                Dim I As Integer
                'FindChanges()
                '                Me.odbcDatabase.InsertCommand.Connection = OleDbConnection1
                Me.odbcDatabase.InsertCommand = New System.Data.OleDb.OleDbCommand("", Me.OleDbConnection1)
                ...
                For Each Item As SkyCD_Simple.skycd_simple.scdItem In Me.Buffer.Items
                    ...
                    I = I + 1
                    If I Mod 3 = 15 Then System.Windows.Forms.Application.DoEvents()
                    Try
    ...
    Public Function FindChanges() As SkyCD_Simple.skycd_simple
        Static C() As System.IO.FileInfo = (New System.IO.DirectoryInfo(Me.Path)).GetFiles("*.*")
        Dim files() As System.IO.FileInfo = (New System.IO.DirectoryInfo(Me.Path)).GetFiles("*.*")        
        If files.Length > C.Length Then            
            ...
            For I = 0 To files.Length - 1
                ...
                System.Windows.Forms.Application.DoEvents()
            Next
            C = (New System.IO.DirectoryInfo(Me.Path)).GetFiles("*.*")
            Return Nothing
        End If
        Return Nothing
    End Function

    Public Function IsInArray(ByVal C() As System.IO.FileInfo, ByVal el As System.IO.FileInfo) As Boolean
        Dim i As Integer
